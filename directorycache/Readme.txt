...
getChildren() manages the cache of directory entries and fetches children lazily, that is, as needed/on demand. 
...
In getChildren(), we preserve the i=0 case that is pulled out separately from the rest of the code because we need to assign the first child of the list to a DirectoryEntry called **prev**, which will be used within the loop as we extract the children Directory Entries. 
